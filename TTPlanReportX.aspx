<%@ Page Title="" Language="VB" MasterPageFile="~/Empty.master" AutoEventWireup="false" CodeFile="TTPlanReportX.aspx.vb" Inherits="TTPlanReportX" MaintainScrollPositionOnPostback="true" %>
...
<div runat="server" id="divform" class="pg" style="min-height:750px;">
	<topyca:pageheader runat="server" ID="PageHeader1" Header="" ButtonJava="window.open('CustEventReportuS.Aspx','_self');" ButtonText="חזרה" />
<div runat="server" id="divbuttons">
	...
	<div id="divbuttons1">
		...
		<input type="button" onclick="window.open('/videos/mt.swf','_blank','toolbar=no,location=no,status=yes,menubar=no,scrollbars=yes,alwaysRaised=yes,resizable=yes,top=0,height=800,width=900');" value="סרטון הדרכה" style="display:none;" />
	</div>
</div>
...
WHERE CustEventID = @CustEventID" >
	...
	<asp:SqlDataSource runat="server" ID="DSWorkPlanReport" 
		...
		InsertCommand="INSERT INTO [TT_WorkPlanReport] ([CstpID],[CustEventID], [WorkPlanID], [TxtWhen], [TxtWhat], [UserID], [Loadtime]) VALUES (@CstpID, @CustEventID,@WorkPlanID, @TxtWhen, @TxtWhat, @UserID, GETDATE())" 
		SelectCommand="SELECT R.ID as ID,r.CstpID,r.WorkPlanID,p.Purpose,p.Description,p.Criteria,r.TxtWhen, [TxtWhat],r.UserID,r.Loadtime,u.URName
						...
						LEFT OUTER JOIN (SELECT * FROM dbo.TT_fnWorkPlan(@CstpID)) p ON p.WpID=r.WorkPlanID
						LEFT OUTER JOIN p0t_NtB u ON u.UserID=r.UserID
						Where CustEventID=@CustEventID
						" 
		UpdateCommand="UPDATE [TT_WorkPlanReport] SET [WorkPlanID] = @WorkPlanID, [TxtWhen] = @TxtWhen, [TxtWhat] = @TxtWhat, [UserID] = @UserID, [Loadtime] = GETDATE() WHERE [ID] = @ID" >
		<DeleteParameters>
