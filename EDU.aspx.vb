...
Partial Class EDU
    ...
    Function CheckMust(Optional ByVal sT As String = vbNullString) As Boolean
        ...
        If lblerr.Visible And (sT = vbNullString Or sT = lblerr.Text) Then
            ...
            Response.Write("<script type='text/javascript'>alert('שים לב, למועמד " & hdncustname.Value & " יש פעולות חובה שלא סומנו')</script>")
        End If
        Return b
    End Function
    ...
    Protected Sub TBDATE_PreRender(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim tb As TextBox = CType(sender, TextBox)
        If tb.Text = vbNullString Then
            tb.Text = Format(Now(), "dd/MM/yy")
            tb.ForeColor = Drawing.Color.DeepSkyBlue
        End If
    End Sub
    ...
    Protected Sub GridView1_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView1.PreRender
        ...
        If gv.Rows.Count > 1 Then
            ...
            End If
            If b Then
                If LSBCUST.SelectedIndex > 0 Then hdncustname.Value = LSBCUST.SelectedItem.Text
            End If
        End If
    End Sub
    ...
    Protected Sub btnCER_Click(sender As Object, e As System.EventArgs) Handles btnCER.Click
        If LSBCUST.SelectedValue IsNot Nothing Then
            Session("LastCustID") = LSBCUST.SelectedValue
            Session("LeftToo") = 1
            Response.Redirect("CustEventReport.aspx")
        End If
    End Sub

    Protected Sub btnCER_PreRender(sender As Object, e As System.EventArgs) Handles btnCER.PreRender
        Dim btn As Button = CType(sender, Button)
        Dim gv As GridView = GridView1
        If gv.Rows.Count > 0 Then
            Dim gvr As GridViewRow = gv.Rows(gv.Rows.Count - 1)
            Dim hdn As HiddenField = CType(gvr.FindControl("hdnOKK"), HiddenField)
            If hdn IsNot Nothing Then
                btn.Enabled = (lblerr.Visible = False) And hdn.Value = 0
            End If
        End If
    End Sub

    Protected Sub HLUPD_PreRender(sender As Object, e As System.EventArgs) Handles HLUPD.PreRender
        Dim hl As HyperLink = CType(sender, HyperLink)
        If LSBCUST.SelectedValue <> vbNullString Then
            hl.NavigateUrl = "CustomerUPD2.aspx?CAND=1&CustomerID=" & LSBCUST.SelectedValue
            hl.Text = "עדכון פרטי מועמד"
        Else
            hl.Text = vbNullString
            hl.NavigateUrl = vbNullString
        End If
    End Sub
End Class
