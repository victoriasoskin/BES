...
Partial Class SurveysNOID
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        WLog(35)
        If rootElement Is Nothing Then rootElement = XElement.Load(MapPath("App_Data/" & "ES2011.xml")) 'Request.QueryString("F")))
        ...
        If Not IsPostBack Then
            ...
            Dim DED As DateTime = Now()

            Dim cI As New SqlCommand("", dbConnection)
            cI.CommandText = "insert into Survey_Forms(SurveyID,loadTime) Values(" & iFtype & ",'" & Format(DED, "yyyy-MM-dd HH:mm:ss") & "')"
            dbConnection.Open()
            cI.CommandType = Data.CommandType.Text
            cI.ExecuteNonQuery()

            'Get new form FormID
            cI.CommandText = "Select FormID From Survey_Forms where SurveyID = " & iFtype & " And loadTime = '" & Format(DED, "yyyy-MM-dd HH:mm:ss") & "'"
    ...
    Protected Sub rbla_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Else
            Try
                Dim conCompU As New SqlCommand("update Survey_Answers set Val=" & rbl.SelectedValue & " Where FormID=" & FormNo & " And QuestionID=" & hdnid.Value, dbConnection)
 ...
 Into cT = Sum(If(p.Val Is Nothing, 1, 0)) _
  Select cT
        ...
        If (tcount(0) > 0 And Left(LBLERR.Text, 2) <> "יש") Or (mustc + iFrameSelected > 0) Then
            ...
            Dim s1 As String = "יש עוד " & tcount(0) & " שאלות שלא נענו<br/>"
            's = "יש עוד " & tcount(0) & " שאלות שלא נענו(מסומנות עם רקע סגול). <br/>עמודים עם שאלות שלא נענו: <br/>"
   ...
   Select New With {.id = qE.Attribute("id").Value}
    ...
    Protected Sub rbla_q_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles rbla_q.SelectedIndexChanged
