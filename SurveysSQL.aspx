<%@ Page Title="" Language="VB" Debug="true" MasterPageFile="~/Surveys.master" AutoEventWireup="false" CodeFile="SurveysSQL.aspx.vb" Inherits="SurveysSQL" enableViewStateMac="false" validateRequest="false" enableEventValidation="false" viewStateEncryptionMode ="Never"  %>
...
<%@ Import Namespace="WRCookies" %>
<%@ Import Namespace="BesConst" %>
...
<script type="text/javascript">
   ...
   function fnWait() {
   	var x = document.getElementById("divwait");
   	x.style.display = 'block';
   }
</script>
<div runat="server" id="divrebkd" visible="false" style="position:fixed;top:20%;right:40%;background-color:White;border:2px outset Gray;text-align:center;width:200px;height:100px;" onmouseup="this.style.display='none';">
     <br /><asp:Label runat="server" ID="lblmsgrel" style="text-align:right;" Text="בוצע שחזור אוטומטי"></asp:Label><br /><br />
...
<div id="divwait" style="position:fixed;top:30%;left:40%;display:none;">
<img id="imgwait" src="images/wait.gif" alt="המתן" />
</div>
        ...
        <tbody>
            <tr>
                ...
                <td runat="server" colspan='<%#If(Eval("IsGroupHeader")=0,if(Eval("FullWidthQuestion")=0,1,3),4) %>' style='<%# "border-top:1px Gray solid;background-color:Silver;width:33%;vertical-align:text-top;" & Eval("style") %>' >
                    <%#Eval("txt")%>
					<asp:Label runat="server" ID="lblMaxCount" Text='<%# Eval("MaxCount") %>' OnPreRender="lblMaxCount_PreRender" style="display:none;" />
                </td>
                <td runat="server" style='<%# Eval("Answerstyle") & "width:60%;" & "border-top:1px Gray solid;"  & If(Eval("IsGroupHeader")=1 Or Eval("FullWidthQuestion")=1,False,True) %>' >
 <%--                   <asp:UpdatePanel runat="server" ID="up_q" UpdateMode="Always" >
                        <ContentTemplate>
--%>                        
							<asp:HiddenField ID="HDNParent" Value='<%#Eval("ParentQuestion") %>' runat="server" />
                                    ...
                                    <tr style="width:100%">
                                        <td style="vertical-align:top;width:50%;">
                                            ...
                                            <asp:DataList runat="server" ID="DL1" OnDataBinding="DL_DataBinding">
                                                <ItemTemplate>
                                                    <asp:CheckBox runat="server" ID="cbf" Text='<%# Eval("FrameName") %>' OnCheckedChanged="cb_CheckedChanged" onclick="SingleSelect('cbf',this);" />
...
--%>                            <asp:TextBox ID="TBDET_S" AutoPostBack="false"  OnTextChanged="tb_OnTextChanged" runat="server" CssClass="tbw" TextMode="MultiLine" Rows="4" Width="450px" Visible='<%#Eval("tb") <> vbnullstring %>' Text='<%#Eval("textdet") %>'></asp:TextBox>
								<asp:CheckBox runat="server" ID="cbYes" Visible="false" OnCheckedChanged="cbYes_CheckedChanged" Checked='<%#if(IsdbNull(Eval("ans")),0,Eval("ans")) %>' OnPreRender="cbYes_PreRender" />
<%--                        </ContentTemplate>
                    </asp:UpdatePanel>
--%>                </td>
...
<div runat="server" id="divlvp" visible="false" dir="rtl">
 ...
 --%> <div>
    <table width="100%">
        <tr>
            ...
            <td>
                <asp:Button ID="Button2A" runat="server" Text="הפרק הקודם" Visible="true" />  <%--OnClientClick="fnWait()" />--%>
            </td>
            <td>
                <asp:Button ID="Button3A" runat="server" Text="הפרק הבא" Visible="true" /> <%--OnClientClick="fnWait()" />--%>
            </td>
            <td>
  ...
  <div>
    ...
    ConnectionString="<%$ ConnectionStrings:book10VPS %>" SelectCommand="SV_lvqblk" 
        ...
        <SelectParameters>
            ...
            <asp:QueryStringParameter DefaultValue="1" Name="LanguageID" 
                QueryStringField="L" Type="Int32" />
        </SelectParameters>
</asp:SqlDataSource>
  <asp:SqlDataSource ID="DSSERVICES" runat="server" 
    ConnectionString="<%$ ConnectionStrings:book10VPS %>" SelectCommand="SELECT x.ServiceID as SID1,x.ServiceName AS SN1,z.ServiceID AS SID2,z.ServiceName as SN2
                                                                        ...
                                                                        From ServiceList
                                                                        WHERE ServiceID in (SELECT DISTINCT ServiceID From vSurvey_Frames Where SurveyID=@SurveyID) ) a
                                                                        WHERE  n % 2 = 1) x
                                                                        left outer join (
