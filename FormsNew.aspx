<%@ Page Title="" Language="VB" Debug="true" MasterPageFile="~/Sherut.master" MaintainScrollPositionOnPostback="true" AutoEventWireup="false" CodeFile="FormsNew.aspx.vb" Inherits="FormsNew" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div runat="server" id="divform" class="pg">
        ...
        <asp:Panel runat="server" Style="max-height: 770px;" Width="800px" ScrollBars="Auto">
            <asp:UpdatePanel runat="server" ID="up_q">
                <ContentTemplate>
                    ...
                    <asp:HiddenField runat="server" ID="hdnMarkHigh" Value="0" />
                            ...
                            <ItemTemplate>
                                <tr>
                                    <td style="background-color: Silver; border: 1px dotted Gray;">
                                        <asp:Label ID="lblid_q" runat="server" Text='<%#Eval("id", "{0:#}")%>' Width="10px"></asp:Label>
                                    </td>
                                    <td style='<%# String.Format("background-color: Silver; border-top: 1px dotted black; border-bottom: 1px dotted black;{0}", If(Eval("subtitle") = 0, "", "font-weight:bold;"))%>' colspan='<%#If(Eval("subtitle") = 0,1,2) %>'>
                                        <asp:Label ID="lbltxt_q" runat="server" Text='<%#"<div>" & Eval("txt") & "</div>"%>' Width="120px" ToolTip='<%#Eval("ToolTip")%>'></asp:Label>
                                    </td>
                                    <td style='<%#Eval("style")%>'>
                                        ...
                                        <asp:TextBox runat="server" ID="tbanswer" TextMode="MultiLine" Rows="4" Width="100%" Visible='<%#Eval("tb") %>' OnTextChanged="tbanswer_TextChanged" Text='<%#Eval("TextDet") %>' />
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:ListView>
<%--                     <asp:TextBox runat="server" ID="tbanswer" TextMode="MultiLine" Rows="4" Width="100%" Visible='true' OnTextChanged="Summery_TextChanged" Text='<%#Eval("TextDet") %>' />--%>
                    ...
                    <div runat="server" id="diveduResults" visible="false">
                        <table>
                            <tr>
                                <td>
                                    <asp:ListView runat="server" ID="lveduResults" Visible="false">
                                        <LayoutTemplate>
                                            <table>
                        ...
                        <div style="position: relative;">
                            <asp:Button runat="server" Text="חזרה לגרף ראשי" ID="btnbacktomainchart" OnClick="btnbacktomainchart_Click" Style="position:absolute;top:0px;left:5px;" Visible="false" />
                            <asp:Chart runat="server" ID="chrtG" Width="795px" Height="350px" OnClick="chrtG_Click">
                            </asp:Chart>
                        </div>
                    </div>
    ...
    <div runat="server" id="divalert" visible="false">ששששששששששששששש</div>
    ...
    <asp:SqlDataSource ID="dsbuttons" runat="server" ConnectionString="<%$ ConnectionStrings:BEBook10 %>" SelectCommand="SELECT [gid], ISNULL(ShortName,[Name]) sName,Name FROM [p5t_FormQuestionGroups] WHERE ([EventTypeId] = @EventTypeId)">
        <SelectParameters>
            <asp:QueryStringParameter Name="EventTypeId" QueryStringField="EVT" Type="Int32" />
        </SelectParameters>
    </asp:SqlDataSource>
</asp:Content>
