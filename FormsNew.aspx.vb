...
Partial Class FormsNew
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        FormTypeID = u.selectDBScalar(String.Format("SELECT FormId FROM Book10_21.dbo.p5t_FormTypes WHERE CustEventTypeID = {0}", Request.QueryString("EVT")))
        ...
        If Not IsPostBack Then
            If ViewState("iStatus") = 0 Then
                Dim firstGroup As String = u.selectDBScalar(String.Format("SELECT top 1 Name FROM p5t_FormQuestionGroups WHERE EventTypeID = {0} ORDER BY gid", Request.QueryString("EVT")))
                hdnCurrentPage.Value = firstGroup 'If(Request.QueryString("EVT") = 139, "טיפול עצמי ADL", If(Request.QueryString("EVT") = 140, "טיפול עצמי ADL", "חברתי - קבוצת השווים"))
                Dim b As Boolean = u.selectDBScalar(String.Format("SELECT ISNULL(MarkHigh,0) FROM  p5t_FormTypes WHERE CUstEventTypeID = {0}", Request.QueryString("EVT")))
                hdnMarkHigh.Value = b
            Else
                hdnCurrentPage.Value = "תוצאות"
            End If
            ssD()
        End If
    End Sub
    ...
    Sub ssD(Optional ByVal xE As XElement = Nothing)
        Dim sql As String = "SELECT g.sType,q.Questionid As Id,q.style,ISNULL(q.AnswerGroupId,0) AnswerGroupId,ISNULL(q.tb,0) tb,q.txt,a.val ans,t.TextDET,a.AnswerID As ansId,ISNULL(q.sumGroup,'')sumGrp,q.FirstQuestion fQ,q.LastQuestion lQ,q.ToolTip,ISNULL(q.subtitle,0) subtitle , ft.formId as FormTypeId" & vbCrLf & _
                            ...
                            "left outer join book10_21.dbo.p5t_formTypes ft on ft.custeventtypeid= g.eventtypeid" & vbCrLf & _
                            "WHERE g.EventTypeId = " & Request.QueryString("EVT") & " And g.Name = '" & hdnCurrentPage.Value & "'" & vbCrLf & _
                            "ORDER BY q.Ord"
        ...
        Dim formTypeId As Integer = dt.Rows(0)("formTypeId")
        ...
        Select Case sType
            ...
            Case "eduResults"
                ...
                sSql = "DELETE FROM p5t_FormResults WHERE FormID = " & FormNo & _
                  ...
                  " FROM FR_FormCalc c " & _
                      ...
                      " WHERE(FormTypeID = " & formTypeId & ") " & _
                      " ORDER BY Grp " & _
                      " INSERT INTO p5t_FormResults(FormID,grp,perc,gid) " & _
                      " SELECT " & FormNo & ",'ממוצע',Avg(perc),999 " & _
                ...
                '     End If
                '
                sSql = "SELECT grp, perc, Ld,Typ,Ord FROM (" & _
                    " SELECT grp,perc,gid,LoadTime As Ld,'a' Typ,0 Ord FROM p5t_Forms f " & _
                ...
                If btnVersions.Text = "הצג גם גרסאות קודמות" And IsPostBack Then sSqlVersions = " UNION ALL " & _
                        " SELECT grp,perc,gid,LoadTime AS Ld,'a' Typ,2 Ord FROM p5t_Forms_Versions f " & _
                        " LEFT OUTER JOIN p5t_FormResults_Versions v ON f.FormID = v.FormID " & _
                        " WHERE OrgFormID = " & FormNo

                sSqlOrder = ") x ORDER BY Ord,Ld Desc,gid "
            ...
            Case "resResults"
                ...
                dbConnection.Open()
                Dim sSql As String = vbNullString
                Dim sSqlChild As String = vbNullString
                Dim sSqlVersions As String = vbNullString
                Dim sSqlOrder As String = vbNullString
                ...
                sSql = "DELETE FROM p5t_FormResults WHERE FormID = " & FormNo & _
                    " INSERT INTO p5t_FormResults(FormID,grp,perc,gid) " & _
                    " SELECT " & FormNo & ",Name,Weight/SUM(Weight) OVER() * a.Val * 4 , Grp " & _
                  " FROM FR_FormCalc c " & _
                      " OUTER APPLY (SELECT AVG(CAST(Val AS float)) As Val FROM p5t_Answers WHERE FormID=" & FormNo & " AND QuestionID Between c.firstQuestion AND c.lastQuestion) a " & _
                      " WHERE(FormTypeID = " & FormTypeID & ") " & _
                      " ORDER BY Grp " & _
                      " INSERT INTO p5t_FormResults(FormID,grp,perc,gid) " & _
                      " SELECT " & FormNo & ",'ממוצע',SUM(perc)/7,99 " & _
                      "FROM p5t_FormResults WHERE FormID= " & FormNo
                cD.CommandText = sSql
                cD.ExecuteNonQuery()
                Session("Colsing") = 0
                ...
                Dim ChildFormNo As Integer = u.selectDBScalar(String.Format("SELECT CustRelateID FROM CustEventList WHERE CustEventID = (SELECT ISNULL(MAX(CustEventID),0) FROM CustEventList WHERE CustomerId = {0} AND CustEventTypeID = (SELECT ChildForm FROM p5t_FormTypes WHERE CustEventTypeID= {1}))", ReadCookie_S("LastCustID"), Request.QueryString("EVT")))
                ...
                If ChildFormNo <> 0 Then
                    sSqlChild = " UNION ALL " & _
    " SELECT grp,perc,gid,LoadTime As Ld,'לקוח' Typ,1 Ord FROM p5t_Forms f " & _
    " LEFT OUTER JOIN p5t_FormResults r ON r.FormID = f.FormID " & _
    " WHERE f.FormID = " & ChildFormNo & " "

                End If


                If btnVersions.Text = "הצג גם גרסאות קודמות" And IsPostBack Then sSqlVersions = " UNION ALL " & _
                        " SELECT grp,perc,gid,LoadTime Ld,'a' Typ,2 Ord  FROM p5t_Forms_Versions f " & _
                        " LEFT OUTER JOIN p5t_FormResults_Versions v ON f.FormID = v.FormID " & _
                        " WHERE OrgFormID = " & FormNo

                sSqlOrder = ") x ORDER BY Ord,Ld Desc,gid "

                cD.CommandText = sSql & sSqlChild & sSqlVersions & sSqlOrder
                da = New SqlDataAdapter(cD)
                dt = New DataTable()
                da.Fill(dt)
                diveduResults.Visible = True
                ...
                da = New SqlDataAdapter(cD)
                da.Fill(dt)
                btnbacktomainchart.Visible = False
                ShowChart(dt)
                dbConnection.Close()
            ...
            Case "workResults"
                ...
                sSql = "DELETE FROM p5t_FormResults WHERE FormID = " & FormNo & _
                  ...
                  " FROM FR_FormCalc c " & _
                      " OUTER APPLY (SELECT AVG(CAST(Val AS float)) As Val FROM p5t_Answers WHERE FormID=" & FormNo & " AND QuestionID Between c.firstQuestion AND c.lastQuestion) a " & _
                      " WHERE(FormTypeID = 6) " & _
                      " ORDER BY Grp " & _
                      " INSERT INTO p5t_FormResults(FormID,grp,perc,gid) " & _
                      " SELECT " & FormNo & ",'ממוצע',SUM(perc)/4,99 " & _
                      "FROM p5t_FormResults WHERE FormID= " & FormNo
                cD.CommandText = sSql
                cD.ExecuteNonQuery()
                Session("Colsing") = 0
                ...
                '
                sSql = "SELECT grp, perc, Ld,Typ,Ord FROM (" & _
                    " SELECT grp,perc,gid,LoadTime As Ld,'s' Typ,0 Ord FROM p5t_Forms f " & _
                    " LEFT OUTER JOIN p5t_FormResults r ON r.FormID = f.FormID " & _
                    " WHERE f.FormID = " & FormNo
    ...
    End Sub
    Sub ShowChart(dt As DataTable, Optional ByVal sCharType As String = "Column", Optional ByVal sURL As String = vbNullString, Optional bDrillGraph As Boolean = False)
        ...
        For Each mRow As DataRow In dt.Rows
            ...
            ' series  type
            Dim sTyp As String = mRow.Item(3)
            sTyp = If(sTyp = "a", vbNullString, sTyp)

            ' Build Series of gid

            Dim sName As String = If(IsDBNull(mRow.Item(0)), vbNullString, mRow.Item(0)) ' Gid Name of Group
            Dim sVer = If(IsDBNull(mRow.Item(2)), "", Format(mRow.Item(2), "dd/MM/yyyy"))
            s = mRow.Item(3) & sVer
            ...
            If srs Is Nothing Then
                ...
                chrtG.Series(s).LegendText = sTyp & " " & sVer
            ...
            If Not bDrillGraph Then chrtG.Series(s).Points.Last.PostBackValue = sName

            ' Add Column label

            If s = Firstver Or sTyp <> vbNullString Then chrtG.Series(s).Points.Last.Label = If(bDrillGraph, Format(dK, "0.##"), Format(dK, "0.00"))
    ...
    Protected Sub BTNCLS_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTNCLS.Click
        ...
        cU.CommandText = String.Format("SELECT distinct g.shortName FROM p5t_Answers a LEFT OUTER JOIN p5t_FormsQuestions q ON q.QuestionId=a.QuestionID LEFT OUTER JOIN p5t_FormQuestionGroups g ON g.gid=q.gid where FormID={0}  AND Val IS NULL", FormNo)
        dbConnection.Open()
        Dim dr As SqlDataReader = cU.ExecuteReader
        Dim b As Boolean = True
        Dim lvi As ListViewItem
        For Each lvi In lvbuttons.Items
            Dim btxn As Button = CType(lvi.FindControl("btngid"), Button)
            btxn.BackColor = Drawing.ColorTranslator.FromHtml("#C0C0C0")
        Next
        While dr.Read
            b = False
            Dim sF As String = dr("ShortName")


            For Each lvi In lvbuttons.Items
                Dim btxn As Button = CType(lvi.FindControl("btngid"), Button)
                If btxn.Text = sF Then
                    btxn.BackColor = Color.Plum
                    Exit For
                End If
            Next
        End While
        ...
        If Not b Then
            Dim sMsg As String = "לא ניתן לסגור שאלון שיש בו שאלות שלא נענו.\n הדפים שיש בהם שאלות שלא נענו, מסומנים."
            ScriptManager.RegisterStartupScript(Page, Me.GetType(), "Key", String.Format("alert('{0}');", sMsg), True)
            Exit Sub
        Else
        End If
        ...
        Next
        '  l = qEvent(0)
        s = "FormsNew.aspx?F=" & Request.QueryString("F") & "&ID=" & l & "&EVT=" & Request.QueryString("EVT")
        bauto = True
        Response.Redirect(s)
        '' '' '' ''End If
    End Sub
    ...
    Protected Sub lblSumGroup_PreRender(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If IsNumeric(sF) And IsNumeric(sL) Then
            Dim connStr As String = ConfigurationManager.ConnectionStrings("BEBook10").ConnectionString
            Dim dbConnection As System.Data.IDbConnection = New System.Data.SqlClient.SqlConnection(connStr)
            Dim cD As New SqlCommand("SELECT SUM(Case WHEN val < 0 THEN 0 ELSE val END) AS val FROM p5t_Answers Where FormID=" & FormNo & " AND QuestionID BETWEEN " & sF & " AND " & sL, dbConnection)
            dbConnection.Open()
    ...
    Protected Sub rbla_DataBinding(sender As Object, e As System.EventArgs)
        ...
        'rbl.BackColor = Drawing.Color.Transparent
        lvi = CType(rbl.NamingContainer, ListViewItem)
        lv = CType(lvi.NamingContainer, ListView)
        s = Right(lv.ID, 1)
        hdn = CType(lvi.FindControl("HDNANS_" & s), HiddenField)
        If rbl.SelectedIndex < 0 And hdn.Value IsNot Nothing And hdn.Value <> vbNullString Then
            rbl.SelectedValue = hdn.Value
        End If
        If lblstatus.Text = " (סגור)" Then
    ...
    Protected Sub btngid_PreRender(sender As Object, e As System.EventArgs)
        Dim btn As Button = CType(sender, Button)
        If Not btn.BackColor = Color.Plum Then
    ...
    Protected Sub lv_q_PreRender(sender As Object, e As EventArgs) Handles lv_q.PreRender
        If hdnMarkHigh.Value Then
            Dim lv As ListView = CType(sender, ListView)
            Dim val As Int16 = -10
            For Each lvi As ListViewItem In lv.Items
                Dim rbl As RadioButtonList = CType(lvi.FindControl("rbla_q"), RadioButtonList)
                Dim hdn As HiddenField = CType(lvi.FindControl("HDNANS_q"), HiddenField)
                If rbl IsNot Nothing Then
                    If rbl.SelectedValue <> vbNullString Then
                        val = If(val > CInt(rbl.SelectedValue), val, CInt(rbl.SelectedValue))
                    ElseIf hdn.Value <> vbNullString Then
                        val = If(val > CInt(hdn.Value), val, CInt(hdn.Value))
                    End If
            ...
            For Each lvi As ListViewItem In lv.Items
                ...
                If rbl IsNot Nothing Then
                    If rbl.SelectedValue <> vbNullString Then
                        If val <= CInt(rbl.SelectedValue) Then rbl.BackColor = Color.Violet Else rbl.BackColor = Color.Transparent
                    Else
                        If ViewState("Err") IsNot Nothing And ViewState("Err") = -1 Then
                            If rbl.SelectedIndex < 0 Then
                                rbl.BackColor = Drawing.Color.Plum
    ...
    Protected Sub tbanswer_TextChanged(sender As Object, e As EventArgs)
        Dim tb As TextBox = CType(sender, TextBox)
        ...
        If sz = vbNullString Or sz = "0" Then
            Try
                ...
                ConCompI.CommandType = Data.CommandType.Text
                ConCompI.ExecuteNonQuery()
                ConCompI.CommandText = "insert into p5t_Texts (FormID,QuestionID,TextDet) values(" & FormNo & "," & hdnid.Value & ",'" & tb.Text.Replace("'", "''") & "')"
                ConCompI.ExecuteNonQuery()
            Catch ex As Exception
                ...
                SendErrMail("נתון דפוק בתשובות, שורה 1337", sBody + ex.Message)
                Response.Redirect("ge.aspx")
                Throw ex
            End Try
        Else
            Try
                Dim conCompU As New SqlCommand("update p5t_p5t_Texts set TextDet='" & tb.Text.Replace("'", "''") & "' Where FormID=" & FormNo & " And QuestionID=" & hdnid.Value, dbConnection)
                conCompU.CommandType = Data.CommandType.Text
                conCompU.ExecuteNonQuery()
            Catch ex As Exception
                Throw ex
    ...
    End Sub
    Protected Sub Summery_TextChanged(sender As Object, e As EventArgs)
        Dim tb As TextBox = CType(sender, TextBox)
        ...
        If text = vbNullString Or text = "0" Then
            Try
                Dim ConCompI As New SqlCommand("delete from p5t_Texts where QuestionID=" & text & " AND FormID=" & FormNo, dbConnection)
                ConCompI.CommandType = Data.CommandType.Text
                ConCompI.ExecuteNonQuery()
                ConCompI.CommandText = "insert into p5t_Texts (FormID,QuestionID,TextDet) values(" & FormNo & "," & text & ",'" & tb.Text.Replace("'", "''") & "')"
                ConCompI.ExecuteNonQuery()
            Catch ex As Exception
        ...
        Else
            Try
                Dim conCompU As New SqlCommand("update p5t_Texts set TextDet='" & tb.Text.Replace("'", "''") & "' Where FormID=" & FormNo, dbConnection)
                conCompU.CommandType = Data.CommandType.Text
                conCompU.ExecuteNonQuery()
            Catch ex As Exception
                Throw ex
    ...
    Protected Sub chrtG_DataBinding(sender As Object, e As EventArgs)

    End Sub

    Protected Sub chrtG_Click(sender As Object, e As ImageMapEventArgs)
        Dim s As String = e.PostBackValue
        If s <> vbNullString Then
            btnbacktomainchart.Visible = True
            Dim sql As String = String.Format("SELECT grp,perc,NULL As Ld,Typ FROM (" +
                                        "SELECT 1 as tord, Ord,txt grp ,a.val perc,'מתאם' Typ " +
                                        ...
                                        "WHERE ((g.Name = N'{0}') OR (g.ShortName = N'{0}')) AND a.FormID={1} AND g.EventTypeID = {2} " +
                                        "UNION ALL " +
                                        "SELECT 1 as tord,99999 ord,grp, perc,'מתאם' Typ " +
                                        "FROM (SELECT grp,perc " +
                                        "FROM Book10_21.dbo.p5t_Forms f " +
                                        "LEFT OUTER JOIN Book10_21.dbo.p5t_FormResults r ON r.FormID = f.FormID " +
                                        "WHERE f.FormID = {1} AND grp = N'{0}' ) y ", s, FormNo, Request.QueryString("EVT"))

            Dim childFormTypeId = u.selectDBScalar(String.Format("SELECT ISNULL(ChildForm,0) FROM p5t_FormTypes WHERE CustEventTypeID= {0}", Request.QueryString("EVT")))
            Dim ChildFormNo As Integer = u.selectDBScalar(String.Format("SELECT CustRelateID FROM CustEventList WHERE CustEventID = (SELECT ISNULL(MAX(CustEventID),0) FROM CustEventList WHERE CustomerId = {0} AND CustEventTypeID = {1})", ReadCookie_S("LastCustID"), childFormTypeId))

            If ChildFormNo <> 0 Then
                sql &= String.Format(" UNION ALL SELECT  2 as tord,Ord,txt grp ,a.val perc,'לקוח' Typ " +
                                        "FROM Book10_21.dbo.p5t_FormsQuestions q " +
                                        "LEFT OUTER JOIN Book10_21.dbo.p5t_Answers a ON a.QuestionID=q.QuestionID " +
                                        "LEFT OUTER JOIN Book10_21.dbo.p5t_FormQuestionGroups g ON g.gid=q.gid " +
                                        "WHERE ((g.Name = N'{0}') OR (g.ShortName = N'{0}')) AND a.FormID={1}  AND g.EventTypeID = {2} " +
                                        "UNION ALL " +
                                        "SELECT 2 as tord,99999 ord,grp, perc,'לקוח' Typ " +
                                        "FROM (SELECT grp,perc " +
                                        "FROM Book10_21.dbo.p5t_Forms f " +
                                        "LEFT OUTER JOIN Book10_21.dbo.p5t_FormResults r ON r.FormID = f.FormID " +
                                        "WHERE f.FormID = {1} AND grp = N'{0}' ) y ", s, ChildFormNo, childFormTypeId)

            End If
            sql &= ") x Order By tord,Ord"

            Dim dt As DataTable = u.selectDBQuery(sql)
            ShowChart(dt, , , True)
        End If
    ...
    Protected Sub btnbacktomainchart_Click(sender As Object, e As EventArgs)
        btnbacktomainchart.Visible = False
        ssD()
    End Sub
End Class
